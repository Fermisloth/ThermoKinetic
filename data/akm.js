 ...
 * Body:
 * {
 *   productType: "mrna_vaccine" | "biologic" | "insulin",
 *   excursions: [
 *     { tempC: 8, durationHours: 4 },
 *     { tempC: 12, durationHours: 1.5 }
 *   ],
 *   shipmentId?: string   (optional — will be generated if not provided)
 * }
 */
router.post(
  "/calculate",
  ...
  (req, res) => {
    ...
    try {
      ...
      // MKT = -Ea/R / ln( Σ(exp(-Ea/RTi)*Δti) / Σ(Δti) )
      const Ea = profile.Ea;
      const R_const = 8.314;
 ...
 * Returns available product types and their profiles (sans internal constants)
 */
